...
def _resolve_schema(proposal):
    ...
    return [(k, FALLBACK_SECTION_LABELS.get(k, k.replace("_", " ").title())) for k in keys]
...
def generate_docx(proposal) -> BytesIO:
    ...
    meta_run = meta_para.add_run(f"{status_label}  •  {datetime.now().strftime('%B %d, %Y')}")
    ...
    footer_run = footer_para.add_run(f"Generated by Draftly  •  {org_name}")
